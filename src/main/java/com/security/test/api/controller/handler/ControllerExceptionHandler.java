...
public class ControllerExceptionHandler {
    ...
    public ResponseEntity<CustomErrorDTO> accessDenied(AccessDeniedException e, HttpServletRequest request) {
        // Se não há autenticação ou é autenticação anônima, retorna 401 (Unauthorized)
        // Se há autenticação mas sem permissão, retorna 403 (Forbidden)
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        if (authentication == null || 
            !authentication.isAuthenticated() || 
            authentication instanceof AnonymousAuthenticationToken ||
            "anonymousUser".equals(authentication.getName())) {
            return buildErrorResponseFromMessage("Token inválido ou ausente", HttpStatus.UNAUTHORIZED, request);
        }
